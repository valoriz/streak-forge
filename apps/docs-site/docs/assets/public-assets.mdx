...
The `public/assets/` directory is where JS and CSS files are placed for runtime loading via `gDom.loadPackage()`. `loadPackage` fetches files from this directory at the `/assets/` URL path by inserting a real `<script src>` or `<link rel="stylesheet">` tag into the page.
...
`loadPackage` always prepends `/assets/` to the path you pass it. A file at `public/assets/js/motion.js` is fetched as `/assets/js/motion.js`:
...
In production, the static host or CDN must serve `public/assets/` at the `/assets/` path. `loadPackage` calls will fail to load packages if this path mapping is not preserved.
