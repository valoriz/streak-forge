...
Data handlers are async functions with the signature `(metadata, { common }) => {...}` that return plain objects. Import the handler, call it, and assert the shape of the return value.

```ts
// src/handlers/HomeDataHandler.ts
export default async function getHomeData(metadata, { common }) {
  return {
    status: 200,
...
describe("HomeDataHandler", () => {
  ...
  beforeAll(async () => {
    // Called with no arguments here since this test doesn't need metadata
    // or a common handler result — real calls receive (metadata, { common }).
