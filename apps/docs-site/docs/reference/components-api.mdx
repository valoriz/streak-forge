...
The same `type` may be used by multiple entries on one page as long as each has a distinct `id` (and its own placeholder). See [WidgetPlaceholder → Reusing a Widget Type](/components/widget-placeholder#reusing-a-widget-type-multiple-times).
